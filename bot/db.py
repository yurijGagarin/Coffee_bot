...
from navigation import get_next_saturday
    ...
    async with async_session() as session:
        result = await session.execute(
            select(Booking).where(
                Booking.user_id == user_id, Booking.order_date == next_saturday
            )
        )
...
async def update_booking_qty(user_id, product_type, math):
    async with async_session() as session:
        ...
        if product_type in booking:
            booking[product_type].qty = max(
                0, min(booking[product_type].qty + diff, user[0].max_to_order)
            )
        else:
